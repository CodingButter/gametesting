...
define(['Entity','Tile','Rectangle'],function(Entity,Tile,Rectangle){
    ...
    var Creature = Entity.extend({
        init:function(_handler,_x,_y,_width,_height,_health,_speed){
            this._super(_handler,_x,_y,_width,_height);
            this.health = _health;
            this.speed = _speed;
            this.xMove = 0;
            this.yMove = 0;
            this.r = Math.max(_width,_height)/2;
            this.m = this.r;
        ...
        collisionWithTile:function(_x,_y){
			var tile = this.handler.getWorld().getTile(_x,_y)
			if(tile)
            return tile.isSolid();
			else return false;
        },
