 ...
 */
define(['Class','TileLoader','Utils','EntityManager','Player','Tree','SpatialGrid','MiniMap'],function(Class,Tile,Utils,EntityManager,Player,Tree,SpatialGrid,MiniMap){

  var totalplayers = 800;
  var World = Class.extend({
        init:function(_path,_handler){
			...
			this.miniMap = new MiniMap(_handler);
            ...
            for(var i=0;i<parseInt(Math.sqrt(totalplayers));i++)
                for(var j=0;j<parseInt(Math.sqrt(totalplayers));j++)
                    this.entityManager.addEntity(new Player(_handler,800+(60*i),1200+(60*j)));
        },
